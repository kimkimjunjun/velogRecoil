...
const RegiModal = () => {
    ...
    return (<div className="flex z-10">
        <div className={`${theme === 'light' ? 'bg-[#F8F9FA]' : 'bg-[#1E1E1E]'} w-[200px] flex-col justify-center z-10`}>
        ...
        <div className={`w-[390px] p-5 ${theme === 'light' ? 'bg-white' : 'bg-[#121212]'} `}>
            <div className='flex flex-col justify-end'>
                ...
                <div className='email'>
                    <h2 className="text-[1.3rem] font-bold mt-6">회원가입</h2>
                </div>
                <section>
                    <h4 className="mt-4 mb-4 text-[#868E96] font-bold">이메일로 회원가입</h4>
                    <form className='w-[100%] flex h-12'>
                        <input className={`flex-1 p-4 border border-gray-300 focus:border-[#96F2D7] rounded-sm text-base focus:outline-none ${theme === 'light' ? 'bg-[#ffffff] text-gray-800' : 'bg-[#1E1E1E] text-white'} `} placeholder='이메일을 입력하세요.' type='text' />
                        <button className={`text-[1rem] font-bold w-[6rem] cursor-pointer ${theme === 'light' ? 'bg-[#12B886] text-white' : 'bg-[#96F2D7] text-black'}`} type='submit'>로그인</button>
                    </form>
                </section>
                <section>
                    <h4 className="mt-10 mb-4 text-[#868E96] font-bold">소셜 계정으로 회원가입</h4>
                ...
                <div className='pt-[40%] flex'>
                    ...
                    <div className=" inline-block font-bold text-[#12B886] cursor-pointer" tabindex="7" data-testid="switchmode" onClick={RegiModalOpen}>로그인</div>
                </div>
